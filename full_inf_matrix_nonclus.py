...
start = time.time()
fftw_lib=ctypes.CDLL('./fftw_wrapper.so')
...
def extractMAG(path_string):
    ...
    for i in range(0,len(data_milli)):
        mag_milli.append(   ( csv_files[i]  ,   ((data_milli[i]['mag']).tolist()) ) )
...
Milli_paths_L = '/home/kali/Desktop/ZTFData/LenseQuasars'
Milli_paths_N = '/home/kali/Desktop/ZTFData/non_Quasars'

#----------------------------------------------------------------------------
#These are the lists will all of the magnitudes
full_mag_list_L=extractMAG(Milli_paths_L)
full_mag_list_N=extractMAG(Milli_paths_N)
#-----------------------------------------------------------------------------------
#Id list
conIDlist=[]
for _,lstC in full_mag_list_L:
    conIDlist.append(_)
nonIDlist=[]
for _,lstN in full_mag_list_N:
    nonIDlist.append(_)
confilenames = [os.path.basename(path) for path in conIDlist]
nonfilenames = [os.path.basename(path) for path in nonIDlist]
#------------------------------------------------------------------------------------------
#creation of con non identifier
con_ones_list = [1] * len(confilenames)
non_zeros_list = [0] * len(nonfilenames)
#-------------------------------------------------------------------------------------------------
# Sample Entropy
con_sampEnt_list=[]
for _,lstC in full_mag_list_L:
    con_sampEnt_list.append((nk.entropy_sample(lstC)[0]))
non_sampEnt_list=[]
for _,lstN in full_mag_list_N:
    non_sampEnt_list.append((nk.entropy_sample(lstN)[0]))
#-----------------------------------------------------------------------------------------------
#Multis-scale Entropy
con_multEnt_list=[]
for _,lstC in full_mag_list_L:
    lstCarr=np.array(lstC)
    con_multEnt_list.append(nk.entropy_multiscale(lstCarr)[0])
non_multEnt_list=[]
for _,lstN in full_mag_list_N:
    lstNarr=np.array(lstN)
    non_multEnt_list.append(nk.entropy_multiscale(lstNarr)[0])
#---------------------------------------------------------------------------------------
#Integral fourier coefficient entropy
con_intEnt_list=[]
for _,lstC in full_mag_list_L:
    con_intEnt_list.append(integration_entropy(lstC))
non_intEnt_list=[]
for _,lstN in full_mag_list_N:
    non_intEnt_list.append(integration_entropy(lstN))
#---------------------------------------------------------------------------------------
#Fourier Coeff WD Normalized
print('How many FP threads for wasserstein Fcoeff')
num_threads_fcoef=int(input())
con_FouWassEnt_list=[]
for _,lstC in full_mag_list_L:
    con_FouWassEnt_list.append(multi_thread_wasslist(lstC,num_threads_fcoef))
non_FouWassEnt_list=[]
for _,lstN in full_mag_list_N:
    non_FouWassEnt_list.append(multi_thread_wasslist(lstN,num_threads_fcoef))
#---------------------------------------------------------------------------------------
...
for mag_l in onlymags_acf_l:
    con_ACFEnt_list.append((nk.entropy_sample(mag_l)[0]))

onlymags_n=[]
for _,lstN in full_mag_list_N:
    onlymags_n.append(lstN)
onlymags_acf_n=lsof_acf(onlymags_n)
non_ACFEnt_list=[]
for mag_n in onlymags_acf_n:
    non_ACFEnt_list.append((nk.entropy_sample(mag_n)[0]))
#-------------------------------------------------------------------------------


# Name of the CSV file
csv_file = '/home/kali/Desktop/ARes/GravLensAl/all_entropy_ideas_timseries/full_ent_mat.csv'

# Header row
header = ['ID','1=Con, 0=Non','ACF_ENT','FOURIER_ENT','INTEGRAL_ENT','MULTISCALE_ENT','SAMPLE_ENT']
...
with open(csv_file, 'w', newline='') as file:
    ...
    for row in zip(confilenames, con_ones_list ,con_ACFEnt_list,con_FouWassEnt_list,con_intEnt_list,con_multEnt_list,con_sampEnt_list):
        writer.writerow(row)
    for row in zip(nonfilenames,non_zeros_list ,non_ACFEnt_list,non_FouWassEnt_list,non_intEnt_list,non_multEnt_list,non_sampEnt_list):
        writer.writerow(row)
